...
/**
 * REST controller for managing {@link com.baubs.familyphotoalbum.domain.Picture}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PictureResource {
        ...
        Optional<Picture> result = pictureRepository
            ...
            .map(existingPicture -> {
                ...
                if (picture.getDescription() != null) {
                    existingPicture.setDescription(picture.getDescription());
                }
     ...
     */
    @GetMapping("/pictures")
    public List<Picture> getAllPictures(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Pictures");
        return pictureRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Picture> getPicture(@PathVariable Long id) {
        ...
        Optional<Picture> picture = pictureRepository.findOneWithEagerRelationships(id);
