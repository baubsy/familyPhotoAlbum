...
export const Widget = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const renderHelper = () => {
    ...
    for (const widget of widgetList) {
      // const widgetProps = JSON.parse('{"text":"test text","text2":"text text2"}');
      const widgetProps = JSON.parse(widget.props);
      /*
      for(const prop of widget.props.split(",")){
        const key = prop.slice(0, prop.indexOf(':') - 1);
        const value = prop.slice(prop.indexOf(":") + 1);
        widgetProps[key] = value;
      }
      */
      if (widget.type === 'Dummy') {
        jsx.push(React.createElement(Dummy, widgetProps));
      }
    }
